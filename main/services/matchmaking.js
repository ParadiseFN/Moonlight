...
express.get(
  ...
  async (req, res) => {
    ...
    const mmData = jwt.sign(
      {
        ...
        key:
          typeof playerMatchmakingKey === "string"
            ? playerMatchmakingKey
            : undefined,
        bucket: bucketId,
        version: `${ver.build}`,
      },
    ...
    return res.json({
      serviceUrl: `${
        process.env.MATCHMAKER_Secure == "true" ? "wss" : "ws"
  ...
  async (req, res) => {
    ...
    try {
      let response = await axios.get(
        `${process.env.MATCHMAKER_Secure == "true" ? "https" : "http"}://${
          process.env.MATCHMAKER_IP
      ...
      if (!user[req.user.accountId]) {
        return functions.createError(
          "errors.Solara.common.matchmaking.user.not_found",
          `User data for account ID "${req.user.accountId}" was not found`,
          [],
          1013,
          "invalid_user",
          404,
    ...
    res.json({
      ...
      attributes: {
        ...
        ALLOWBROADCASTING_b: true,
        SUBREGION_s:
          user[req.user.accountId].playerRegion == "NAE" ? "VA" : "GB",
        DCID_s: "FORTNITE-LIVEEUGCEC1C2E30UBRCORE0A-14840880",
        tenant_s: "Fortnite",
        MATCHMAKINGPOOL_s: "Any",
        STORMSHIELDDEFENSETYPE_i: 0,
